...
private fun part1() =
		Permutations.permutate(listOf(0, 1, 2, 3, 4), Int::compareTo).map { phases ->
			var power = 0
			phases.forEach {
				power = IntCode(input.toMutableList()).runForPhase(it, power)
			}
			power
		}.max()
...
private fun part2() =
		...
		//listOf(9, 8, 7, 6, 5).let { phases ->
			val inputs = phases.mapIndexed { index, phase -> if (index == 0) mutableListOf(phase, 0) else mutableListOf(phase) }.toMutableList()
			val amplifiers = phases.mapIndexed { index, _ -> index to IntCode(input.toMutableList()) }.toMutableList()
			var maybeFinalOutput: Int? = null
			var finalOutput: Int? = null
			do {
...
private fun IntCode.runForPhase(phase: Int, previousPower: Int = 0): Int =
		AtomicInteger().apply {
			run(listOf(phase, previousPower)) { set(it) }
		}.get()

private val testInput = listOf(3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,
		27,4,27,1001,28,-1,28,1005,28,6,99,0,0,5)
private val input = readInput(7).single().split(",").map(String::toInt)
